...
try {
    ...
    if ($search !== '') {
        $stmt = $db->prepare("SELECT username, email, image_url, admin FROM users WHERE LOWER(username) LIKE :search AND username != 'admin' ORDER BY username ASC");
        $stmt->execute([':search' => '%' . strtolower($search) . '%']);
        $users = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } else {
        $stmt = $db->query("SELECT username, email, image_url, admin FROM users WHERE username != 'admin' ORDER BY username ASC");
        $users = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    foreach ($users as &$u) {
